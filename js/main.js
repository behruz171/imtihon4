...
function bos(){
    ...
    massiv.map((item,index) =>{
        ...
        td.textContent = item.ish
        let td1 = document.createElement("td")
        td1.textContent = item.tel
        let td2 = document.createElement("td")
        td2.textContent = item.email
        let td3 = document.createElement("td")
        td3.textContent = item.guruh

        let btn = document.createElement("button")
        btn.setAttribute("onclick",`ozgar(${index})`)
        // item.status == false ? btn.classList.add("btn","btn-outline-success") : btn.classList.add("btn","btn-outline-danger")
